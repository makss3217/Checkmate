...
enum class BoardPosition(val row : Int, val column : Int) {
    ...
    companion object {
        fun from(row : Int, column : Int) : BoardPosition {
            return BoardPosition.values().first() {it.row == row && it.column == column }
        }
    }
}
